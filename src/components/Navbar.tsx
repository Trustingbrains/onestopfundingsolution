...
export default function Navbar() {
  ...
  return (
    ...
    <div className="flex flex-col">
      <span className="text-gold font-serif font-bold text-2xl leading-none">
        OneStop Funding
      </span>
      <span className="text-gold-light text-sm tracking-widest uppercase mt-1">
        Solutions
      </span>
    </div>
          </Link>
        ...
        <div className="hidden md:flex items-center space-x-8">
          ...
          <Link to="/loans#emi-calculator" className="text-gold-light hover:text-gold transition-colors text-sm uppercase tracking-wider">EMI Calculator</Link>
          <Link
            to="/contact"
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden absolute top-full left-0 w-full bg-black border-b border-dark-border py-4 px-4 flex flex-col space-y-4 shadow-2xl">
          <Link to={isHome ? "#home" : "/"} onClick={() => setIsMobileMenuOpen(false)} className="text-gold-light hover:text-gold text-lg">Home</Link>
          <Link to="/#about" onClick={() => setIsMobileMenuOpen(false)} className="text-gold-light hover:text-gold text-lg">About</Link>
          <Link to="/loans" onClick={() => setIsMobileMenuOpen(false)} className="text-gold-light hover:text-gold text-lg">Loans</Link>
          <Link to="/partners" onClick={() => setIsMobileMenuOpen(false)} className="text-gold-light hover:text-gold text-lg">Partners</Link>
          <Link to="/loans#emi-calculator" onClick={() => setIsMobileMenuOpen(false)} className="text-gold-light hover:text-gold text-lg">EMI Calculator</Link>
          <Link
            to="/contact"
